#' Run CNCDriver open chromatin element p-value calculation
#'
...
#' @param openChromatinRegionBedFile The defintion of open chromatin regions in bed file format
#' @param elementKeyWord Default is "ATAC", the keyword of open chromatin annotation in FunSeq2
...
#' 
getOpenChromatinElementPvalueWithPreFilter2<-function(inputFileDir,outputFileDir,
                            openChromatinRegionBedFile,elementKeyWord="ATAC",
    ...
    #####
    ## load ATAC-seq open chromatin regions bed file
    #####
    ...
    elementDF<-fread(elementBedfileName,header=FALSE,sep="\t",data.table=FALSE)
    if(ncol(elementDF)==6){
      cat(sprintf("Load DRM style bed file\n"))
      colnames(elementDF)<-c("chr","posStart","posEnd","geneSymbol","score","method")
    }else{
      colnames(elementDF)<-c("chr","posStart","posEnd","geneSymbol")
    }
    
    ## add +1 for posStart from bed format 
    elementDF$posStart<-elementDF$posStart+1
    
    
    # unique element segments before merge
    elementTmp<-unique(paste(elementDF$chr,elementDF$posStart,elementDF$posEnd,sep="@"))
    
    
    # genes associate with element
    numOfgeneAssociateWithelement<-length(unique(elementDF$geneSymbol))
    
    elementCoord<-strsplit(elementTmp,"@")
    elementCoord<-do.call(rbind,elementCoord)
    
    elementCoord<-data.frame(elementCoord[,1],as.numeric(elementCoord[,2]),as.numeric(elementCoord[,3]),stringsAsFactors = FALSE)
    colnames(elementCoord)<-c("chr","posStart","posEnd")
    
    elementGR=GRanges(seqnames=elementCoord$chr,
                       ranges=IRanges(start=(elementCoord$posStart),
                                      end=(elementCoord$posEnd)),
                       names=paste(elementCoord$chr,paste(elementCoord$posStart,elementCoord$posEnd,sep="-"),sep=":"))
    
    # unique element segments after merge
    elementGR<-reduce(elementGR)
    
    chrName<-as.character(seqnames(elementGR))
    startPos<-start(elementGR)
    endPos<-end(elementGR)
    ...
    #reducedFunseqOutputNCDS$GENEparsed<-parseFunseqGeneFieldelement(field=reducedFunseqOutputNCDS$GENE,keyword=elementKeyWord,useCores=1)
    ...
    if(file.exists(replicationTimingElementBinnedFile)){
      ...
      elementReplicationTimingDF<-makeElementReplicationTiming(replicationTimingDF,elementDF,useCores)
    ...
    if(FALSE){
      ...
      plot(geneDFuniqueelement$signalValue,geneDFuniqueelement$occurence)
      
      kk<-geneDFuniqueelement[geneDFuniqueelement$signalValue<=ss$signalUpper & geneDFuniqueelement$signalValue>=ss$signalLower,]
      points(kk$signalValue,kk$occurence,col="red")
      #points(ss$signalValue,ss$signalValue,col="green",pch=19)
      
      plot(geneDFuniqueelement$signalValue,geneDFuniqueelement$compositeScore)
    ...
    for(i in 1:length(groupType)){
      ...
      geneDFelement<-mclapply(1:length(geneNameVector), function(x){
      ...
      names(geneDFelement)<-geneNameVector
      }
      ...
      if(file.exists(fileName)){
        ...
        geneDFunique<-mclapply(1:length(geneNameVector), function(x){
          ...
          #cat(sprintf("%s\n",x))  
          geneName<-geneNameVector[x]
          geneDFunique[[geneName]]<-geneDFelement[[geneName]][!(duplicated(geneDFelement[[geneName]]$posIndex)),]
          ...
          collapsedDF<-mergeVariantPosition(geneDFelement[[geneName]],sampleSizeFactor,countsCutOff = 2)
      ...
      geneDFuniqueelement<-geneDFunique[!(duplicated(geneDFunique$posIndex)),]
      ...
      if(!file.exists(fileName)){
        ...
        variantTriMutCategoryParsed<-parsePosVariantByTriMutContextWithAnnotation5(geneDFuniqueelement,mutationDistMatrix,useCores=useCores)
      ...
      prefilteredDF<-mergeDF[mergeDF$posIndex %in% geneDFuniqueelement$posIndex,]
    ...
    #######
      ...
      ncdsMutationCheckList<-unique(geneDFuniqueelement$geneSymbol)
    ...
    #######
      ...
      outputDf<-foreach(k=loopSequence, .options.RNG=seedNum, .packages=c("stringr","parallel","plyr","CNCDriver"))  %dorng% {
        ...
        numOfAlteration<-sum(geneDFuniqueelement[geneDFuniqueelement$geneSymbol %in% ncdsMutationCheckList[k],]$occurence)
        ...
        selectedPosition<-unique(geneDFuniqueelement[geneDFuniqueelement$geneSymbol==gname,]$posIndex)
        ...
        if(FALSE){ 
          ...
          plot(geneDFuniqueelement$signalValue,geneDFuniqueelement$compositeScore,pch=19,xlab="",ylab="",cex=0.6)
          ...
          points(geneDFuniqueelementSelected$signalValue,geneDFuniqueelementSelected$compositeScore,col="red",pch=19,cex=0.6)
        ...
        colnames(tmpResult)<-c("elementUnitPos","numOfAlteredElements","elementPos","numOfAlterationPos","numOfAlteration","numOfPatient","compositeDriverScore","numOfAboveCDscore","reSampleNum","pValue")
      ...
      colnames(outputDf)<-c("elementUnitPos","numOfAlteredElements","elementPos","numOfAlterationPos","numOfAlteration","numOfPatient","compositeDriverScore","numOfAboveCDscore","reSampleNum","pValue","qValue")
